...
int _smallerIndexHeap(DynArr *heap, int i, int j, comparator compare)
{	//Our code
	//does not seem to have an option for equal to
	//CHANGE FOR DUPLICATE PRIORITIES
	int test= compare(heap->data[i],heap->data[j]);
	if (test==-1){return i;}
	else if (test==1){return j;}
	return -1; //Failed   could use this is equal
}
...
{		//if they are ordered, then this should be first element
		//Not sure yet if it always starts at index 0
		//int i=0;
		//while(heap[i]== NULL){i++;}
...
void _adjustHeap(DynArr *heap, int max, int pos, comparator compare)
{	//our code
	//printf("Addjust Called");
	int leftChild = 2 * pos + 1; 
	int rightChild = 2 * pos + 2;  //Sets the index of both children
	int small=pos;    //Used to store index of smallest child
	if (rightChild < max) //There are two children
    {	//Need smalles child  left=2i+1 right =2i+2 
		small=_smallerIndexHeap(heap,leftChild,rightChild,compare);
		assert (small !=pos);   //Ensures change was made to small
		if (compare(heap->data[pos],heap->data[small])==1)
		{ 
			//printf("\nAdjust Heap small/pos: %d/%d",small,pos);
