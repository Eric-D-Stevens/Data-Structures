...
{	//Our Code
	struct Task *_left=(struct Task*)left;
	struct Task *_right=(struct Task*)right;
	if (_left->priority < _right->priority){return -1;}
	if (_left->priority > _right->priority){return 1;}
	if (_left->priority == _right->priority){return 0;}
	return 2;// Compare Failed
...
{//our code
	struct Task *temp;
	temp =malloc(sizeof(struct Task));
	int i=0;
	while(i<TASK_DESC_SIZE){temp->description[i]=desc[i];i++;}
...
{
	if (sizeDynArr(heap)==0){printf("LIST IS EMPTY!!!");return;}
  DynArr *temp;
  TaskP task;
  assert(sizeDynArr(heap) > 0);
