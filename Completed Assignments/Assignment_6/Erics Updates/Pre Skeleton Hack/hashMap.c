...
{//Our code
    ...
    if (hashIndex < 0) {printf("nigganigganigganigganigga"); hashIndex += ht->tableSize;}
    ...
    ht->count++;
    if (tableLoad(ht)>LOAD_FACTOR_THRESHOLD){printf("shibidashibak"); _setTableSize(ht,2*(ht->tableSize));}
...
{//Our Code
	float load=(float)(ht->count)/(float)(emptyBuckets(ht));
	return load;
}
