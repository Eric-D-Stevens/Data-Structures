...
{	//Our Code
	...
	int test; test = compare(cur->val,val);
	if (test== 1 || test==0){cur->left = _addNode(cur->left,val);} //going to left sub tree
	if (test== -1){cur->right = _addNode(cur->right, val);}		//going to right sub tree
	return cur;
}
/*
...
{	//Our Code
    struct Node *temp;	//Temp node to act as current node
	temp=tree->root;	//sets temp to the root node
    while(temp !=0) //Loop untill finding a leaf node
	{	//since the loop wont be entered if temp is a leaf, we check for values first
		if (compare(temp->val,val)==-1){temp=temp->right;}	//Moves to the right
		else if (compare(temp->val,val)==1){temp=temp->left;}		//Moves to the left
		else if (compare(temp->val,val)==0){return 1;}		//Moves to the left
	}//While loop
...
{	//Our code
	if(cur->left!= 0){cur->left = _removeLeftMost(cur->left);}
	else{free(cur);return 0;}
	return cur;
}
/*
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
	...
	if(compare(cur->val,val)==-1){cur->right = _removeNode(cur->right,val);}
	else if(compare(cur->val,val)==1){cur->left = _removeNode(cur->left,val);}
	return cur;
}
/*
